...
const TableOpd: React.FC<table> = ({
    ...
    const fetchIkuOpd = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL}/indikator_utama/opd/${kode_opd}/${tahun_awal}/${tahun_akhir}/${jenis}`,
          {
            headers: {
              Authorization: `${token}`,
        ...
        const filteredData =
          result.data?.filter((n: any) => n.iku_active === isActiveFilter) ||
          [];
  ...
  const UpdateStatusIku = async (id: string) => {
    ...
    try {
      ...
      AlertNotification(
        "Gagal",
        `cek koneksi internet/database server, error catch`,
        "error",
      );
    } finally {
      setProses(false);
  ...
  const { cetakPdfIkuOpd } = useCetakIkuOpd(
    IKU,
    nama_opd,
    tahun_awal,
    tahun_akhir,
    ...
    <>
      ...
      <div className="overflow-auto m-2 rounded-t-xl border">
        <table className="w-full">
          <thead>
            <tr
              className={`${TableAktif ? "bg-emerald-500" : "bg-orange-500"} text-white`}
            >
              <th
                rowSpan={2}
                className="border-r border-b px-6 py-3 text-center"
              >
                No
              </th>
              <th
            ...
            <tr
              className={`${TableAktif ? "bg-emerald-600" : "bg-orange-600"} text-white`}
            >
              {tahun_list.map((item: any) => (
          ...
          <tbody>
            {DataNull ? (
              ...
              IKU.filter((item) => item.is_hide === false).map(
                (item, index) => (
                  <tr key={item.indikator_id || index}>
                    <td
                      className={`border-x border-b ${TableAktif ? "border-emerald-500" : "border-orange-500"} py-4 px-3 text-center`}
                    >
                      {index + 1}
                    </td>
                    <td
                      className={`border-b ${TableAktif ? "border-emerald-500" : "border-orange-500"} px-6 py-4`}
                    >
                      <p>{item.indikator || "-"}</p>
                    ...
                    <td
                      className={`border-r border-b ${TableAktif ? "border-emerald-500" : "border-orange-500"} pr-6`}
                    >
                      <div
                    ...
                    <td
                      className={`border-r border-b ${TableAktif ? "border-emerald-500" : "border-orange-500"} px-6 py-4`}
                    >
                    ...
                    {item.target.map((t: Target, index: number) => (
                      <React.Fragment key={index}>
                        <td
                          className={`border-r border-b ${TableAktif ? "border-emerald-500" : "border-orange-500"} px-6 py-4 text-center`}
                        >
                          {t.target || "-"}
                        </td>
                        <td

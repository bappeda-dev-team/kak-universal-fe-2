...
const Table: React.FC<table> = ({
  ...
  useEffect(() => {
    const hiddenIds = Sasaran.filter((item) => item.is_hide === true).map(
      (item) => item.id_pohon,
    );
  ...
  const hiddenPohon = async (method: "POST" | "DELETE", id: number) => {
    const API_URL = process.env.NEXT_PUBLIC_API_URL;
    let url = "";
    if (method === "DELETE") {
      url = `sasaran_opd/unhide/${id}`;
    } else {
      url = `sasaran_opd/hide/${id}`;
    }
    try {
      const response = await fetch(`${API_URL}/${url}`, {
        method: method,
            ...
            {DataNull ? (
              ...
              Sasaran.map((data: Sasaran, index: number) => {
                ...
                const toggleHidden = async (id: number) => {
                  ...
                  await hiddenPohon(isCurrentlyHidden ? "DELETE" : "POST", id);
                  ...
                  <React.Fragment key={index}>
                    {isHidden ? (
                      <tr>
                        ...
                        {/* SEMUA KOLOM LAIN DIGABUNG */}
                        <td
                          colSpan={29}
                          className="border-r border-b border-emerald-500 px-6 py-4 bg-red-400"
                        >
                          <div className="flex items-center justify-start gap-3">
                            <div className="flex items-center gap-2 text-gray-700">
                              <TbAlertCircle className="text-white" />
                            ...
                            <button
                              className="flex items-center gap-1 rounded-full p-1 px-3 bg-blue-400 text-white border border-blue-400 hover:bg-white hover:text-blue-400"
                      ...
                      <>
                        <tr className={`${isHidden && "bg-red-300"}`}>
                          <td
                            ...
                            rowSpan={
                              data.sasaran_opd.length === 0 ? 2 : TotalRow
                            }
                          >
                            {index + 1}
                          </td>

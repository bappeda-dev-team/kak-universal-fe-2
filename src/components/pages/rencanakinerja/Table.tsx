...
export const TablePerencanaan = () => {
  ...
  const hapusRekin = async (id: any) => {
    ...
    try {
      ...
      AlertNotification(
        "Berhasil",
        "Rencana Kinerja Berhasil Dihapus",
        "success",
        1000,
      );
    } catch (err) {
  ...
  if (loading) {
    ...
    return (
      <h1 className="text-red-500 py-3 px-5 text-center">
        Gagal mendapatkan data Rencana Kinerja KAK, periksa koneksi internet
      ...
      <div className="flex items-center justify-between border-b px-5 py-5">
        <div className="flex flex-col">
          <div className="flex items-center gap-1">
            ...
            <h1 className="font-bold text-2xl uppercase text-green-500">
        ...
        <div className="flex flex-col items-end">
          ...
          <RolePill
            roles={User?.roles}
            className="[&>span]:text-sm [&>span]:px-3 [&>span]:py-1"
          />
        </div>
        {/* } */}
      </div>
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-700 text-white">
              ...
              <th className="border-r border-b px-6 py-3 min-w-[600px]">
                Indikator Target Satuan Rencana Kinerja
              </th>
              <th className="border-r border-b px-6 py-3 min-w-[100px]">
                Status
              </th>
                  ...
                  {data.indikator != null ? (
                    <td className="border-r border-b text-center">
                      <div key={index} className="flex flex-col gap-2 p-2">
                        {data.indikator.map(
                          (item: indikator, index: number) => (
                            <React.Fragment key={index}>
                              <div
                                className={`flex flex-col gap-2 bg-green-100 border border-green-600 rounded-lg p-5`}
                              >
                                <div className="flex items-center justify-between gap-1">
                                  ...
                                  {item.targets ? (
                                    item.targets.map(
                                      (t: target, t_index: number) => (
                                        <div
                                          className="flex flex-col border border-green-600 rounded-lg px-4 py-1"
                                          key={t_index}
                                        >
                    ...
                    <div className="flex flex-col justify-center items-center gap-2">
                      {User?.roles != "level_1" && (
                        <>
                          <ButtonSkyBorder
                            className="w-full"
                            onClick={() =>
                              handleEditRekin(data.id_rencana_kinerja)
                            }
                          >
                      ...
                      <ButtonRedBorder
                        ...
                        onClick={() => {
                          AlertQuestion(
                            "Hapus?",
                            "Hapus Rencana Kinerja yang dipilih?",
                            "question",
                            "Hapus",
                            "Batal",
                          ).then((result) => {

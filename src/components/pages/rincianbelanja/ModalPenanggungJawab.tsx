...
export const ModalPenanggungJawab: React.FC<modal> = ({
    isOpen,
    onClose,
    onSuccess,
    kode_opd,
    ...
    const onSubmit: SubmitHandler<FormValue> = async (data) => {
        const payload = {
            //key : value
            ...data,
            aktif_at: data.aktif_at ? `${data.aktif_at}` : null,
            nip: data.nip?.value,
            kode_opd: kode_opd,
        ...
        try {
            ...
            // }
            setProses(true);
            const response = await fetch(`${branding?.api_perencanaan}/pptk/create`, {
                method: "POST",
                headers: {
                    Authorization: `${token}`,
                    "Content-Type": "application/json",
                    ...
                    >
                        <div className="flex flex-col py-3">
                            ...
                            <div className="border border-green-600 px-4 py-2 rounded-lg">
                                ({Data?.kode_subkegiatan || "kode unknown"}) -{" "}
                                {Data?.nama_subkegiatan || "subkegiatan unknown"}
                            </div>
                        </div>
                        <Controller
                            name="nip"
                      ...
                      Kosongkan jika tidak di non aktifkan
                        ...
                        <ButtonRed
                            className="w-full mb-3 flex items-center gap-1"
                            onClick={handleClose}
                        >
                            <TbX />
                            Batal

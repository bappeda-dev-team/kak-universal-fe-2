...
export const Table: React.FC<Table> = ({ nama_opd, kode_opd, tahun }) => {
    ...
    const fetchSasaran = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL_RENAKSI_OPD}/sasaran_opd/all/${kode_opd}/${tahun}`,
          {
            headers: {
              Authorization: `${token}`,
  ...
  return (
    <div className="overflow-auto">
      {DataNull ? (
        <h1 className="font-bold mx-5 py-5">
          Sasaran OPD belum di tambahkan di RENSTRA
        </h1>
      ) : (
          ...
          <div className="my-2" key={data.id || index}>
            ...
            >
              <h1 className="font-semibold">
                {index + 1}. {data.nama_sasaran_opd || "-"}
              </h1>
              <div className="flex items-center">
                <ButtonBlackBorder
                  onClick={() =>
                    handleIndikator(data.nama_sasaran_opd, data.indikator)
                  }
            ...
            <div
              className={`transition-all duration-300 ease-in-out mx-2 p-2 border-x border-b border-emerald-500`}
            >
              <RekinAsn
                id={data.id}
...
export const RekinAsn: React.FC<RekinAsn> = ({
  id,
  sasaran,
  indikator,
  tahun,
  ...
  const hapusRenaksiOpd = async (id: number) => {
    ...
    try {
      const response = await fetch(
        `${API_URL_RENAKSI_OPD}/rencana-aksi-opd/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `${token}`,
      ...
      AlertNotification(
        "Berhasil",
        "Rencana Aksi OPD Berhasil Dihapus",
        "success",
        1000,
      );
  ...
  const syncRenaksiOpd = async (id: string) => {
    ...
    try {
      const response = await fetch(
        `${API_URL_RENAKSI_OPD}/rencana-aksi-opd/sync_jadwal/${id}`,
        {
          method: "POST",
          headers: {
            Authorization: `${token}`,
  ...
  useEffect(() => {
    ...
    const fetchRekinById = async () => {
      ...
      try {
        const response = await fetch(
          `${API_URL_RENAKSI_OPD}/rencana-aksi-opd/${id}/${tahun}`,
          {
            headers: {
              Authorization: `${token}`,
              "Content-Type": "application/json",
  ...
  if (Error) {
    return (
      <React.Fragment>
        <div className="w-full">
          <div className="border px-6 py-4 text-center text-red-500">
            Cek koneksi internet, terdapat kesalahan server backend atau
            database
          </div>
        </div>
      </React.Fragment>
        ...
        <table className="w-full">
          <thead>
            <tr className="text-xm bg-emerald-500 text-white">
              ...
              <td
                rowSpan={2}
                className="border-r border-b px-6 py-3 min-w-[300px] text-center"
              >
                Aksi/Kegiatan
              </td>
              <td
              ...
              <td
                rowSpan={2}
                className="border-r border-b px-6 py-3 min-w-[100px] text-center"
              >
                Anggaran
              </td>
              <td
          ...
          <tbody>
            {Data.length != 0 ? (
                          ...
                          <div className="flex flex-col justify-center items-center gap-2">
                            ...
                            <ButtonRed
                              ...
                              onClick={() => {
                                AlertQuestion(
                                  "Hapus?",
                                  "Hapus Renaksi OPD yang dipilih?",
                                  "question",
                                  "Hapus",
                                  "Batal",
                                ).then((result) => {
              ...
              <tr>
                <td colSpan={11} className="border-r border-b px-6 py-4">

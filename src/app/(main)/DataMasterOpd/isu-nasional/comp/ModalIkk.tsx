...
export const ModalIkk: React.FC<modal> = ({
  ...
  const onSubmit: SubmitHandler<FormValue> = async (data) => {
    ...
    try {
      ...
      let url = "";
      if (jenis === "tambah") {
        url = "isu-nasional/create";
      } else if (jenis === "edit") {
        url = `isu-nasional/update/${Data?.id}`;
      } else {
        url = "";
      }
      ...
      if (result.code === 201 || result.code === 200) {
        AlertNotification(
          "Berhasil",
          `Berhasil ${jenis === "edit" ? "mengubah" : "menambah"} Isu Nasional`,
          "success",
          1000,
        );
        onSuccess();
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50">
        ...
        >
          <div className="w-max-[500px] py-2 border-b text-center">
            <h1 className="text-xl uppercase">{jenis} Isu Nasional</h1>
          </div>
          <form

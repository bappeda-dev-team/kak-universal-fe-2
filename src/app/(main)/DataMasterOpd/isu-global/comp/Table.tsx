...
const Table: React.FC<Table> = ({ kode_opd, Tahun }) => {
    ...
    const fetchOpd = async () => {
      ...
      try {
        const response = await fetch(
          `${branding?.api_perencanaan}/isu-global/findall/${kode_opd}`,
          {
            headers: {
              Authorization: `${token}`,
        ...
        if (result.code === 200) {
          if (result.data === null) {
            ...
            setData(result.data.isu_globals);
          }
        } else {
          setError(true);
          setData([]);
  ...
  const hapusData = async (id: number) => {
    try {
      const response = await fetch(
        `${branding?.api_perencanaan}/isu-global/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `${token}`,
      ...
      if (result.code === 200) {
        ...
        AlertNotification(
          "Berhasil",
          "Data Isu Global Berhasil Dihapus",
          "success",
          1000,
        );
      } else {

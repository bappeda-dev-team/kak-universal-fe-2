...
import { NspkOpdFindall } from "../type";
...
const Table: React.FC<Table> = ({ kode_opd, Tahun }) => {
  const [Data, setData] = useState<NspkOpdFindall[]>([]);
  const [Error, setError] = useState<boolean | null>(null);

  const [DataModal, setDataModal] = useState<NspkOpdFindall | null>(null);
    ...
    const fetchOpd = async () => {
      ...
      try {
        const response = await fetch(
          `${branding?.api_perencanaan}/nspk-opd/findall/${kode_opd}`,
          {
            headers: {
              Authorization: `${token}`,
  ...
  const hapusData = async (id: number) => {
    try {
      const response = await fetch(
        `${branding?.api_perencanaan}/nspk-opd/delete/${id}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `${token}`,
      ...
      if (result.code === 200) {
        ...
        AlertNotification("Berhasil", "Data NSPK OPD Dihapus", "success", 1000);
      } else {
            ...
            <table className="w-full">
              ...
              <tbody>
                {filteredData.length === 0 ? (
                  ...
                  filteredData.map((item: NspkOpdFindall, index: number) => (
                    <tr key={item.id}>
                      {/* No */}
